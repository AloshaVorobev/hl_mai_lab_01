...
protected:
    TestApp() {
        ...
        Config::get().login() = "laba";
        Config::get().password() = "super";
    }
    ~TestApp() {
...
TEST_F(TestApp, TestPerson) {
    ...
    ASSERT_EQ(login_result1.get_first_name(), "Alexey");
    ASSERT_EQ(login_result1.get_last_name(), "Vorobev");
    ...
    ASSERT_EQ(login_result2.get_last_name(), "V");
    ASSERT_EQ(login_result2.get_age(), 22);

    auto name_result1 = database::Person::search("Alexey", "Vorobev");
